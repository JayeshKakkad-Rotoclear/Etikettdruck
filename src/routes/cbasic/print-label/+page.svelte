<script lang="ts">
	...
	async function loadProductData() {
		...
		isLoading = true;
		try {
			const response = await fetch(`/api/cbasic?serialnummer=${encodeURIComponent(serialnummer.trim())}`);
	...
	async function printLabel() {
		...
		isLoading = true;
		try {
			const response = await fetch('/api/cbasic', {
				method: 'PUT',
...
<svelte:head>
	<title>C Basic Etikett Drucken</title>
</svelte:head>

<div class="form-container">
	<div class="form">
		<h1 class="page-title">
			C Basic Etikett Drucken
		</h1>
