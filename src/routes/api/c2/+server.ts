...
async function makeQrPng(data: string, sizePx = 350): Promise<Buffer> {
  // ECC 'M' is usually plenty; tweak margin to match ZPL ^BQN appearance
...
async function sendToPrinter(zplData: string, printerIp?: string): Promise<boolean> {
  ...
  return new Promise((resolve, reject) => {
    ...
    client.connect(PRINTER_PORT, targetIp, () => {
      ...
      // Close shortly after sending; many Zebras don't send responses
      setTimeout(() => {
        client.destroy();
        resolve(true);
      }, 1000);
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  if (serial) {
    const item = await prisma.singleItemC2.findFirst({ where: { serialnummer: serial } });
    if (!item) return json({ found: false });
    return json({ found: true, item });
  } else {
    const allItems = await prisma.singleItemC2.findMany({ orderBy: { id: 'asc' } });
  ...
  try {
    ...
    if (!body.skipPrint) {
      ...
      try {
        ...
        console.log('C2 label sent to printer successfully');
      } catch (error) {
        console.error('Failed to send C2 label to printer:', error);
      }
    }
