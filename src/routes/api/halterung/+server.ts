...
/* =========================
   POST  /api/halterung
   body: { items: Array<{ artikelnummer, artikelbezeichnung, menge }>, skipPrint?: boolean, printerIp?: string }
   ========================= */
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const { items, skipPrint, printerIp } = body as {
      items?: Array<{ artikelnummer: string; artikelbezeichnung: string; menge: number }>;
      skipPrint?: boolean;
      printerIp?: string;
    };
    ...
    // Determine printing mode: single item vs multiple items
    ...
    if (isSingleItem) {
      // Single item: Current format for individual items
      qrContent =
        items.map((i) => `ART: ${i.artikelbezeichnung} \n`).join('') +
        items.map((i) => `ARTN: ${i.artikelnummer} \n`).join('') + 
        items.map((i) => `MENGE: ${i.menge} \n`).join('') +
        `DAT: ${formattedDate} \n`;
    } else {
      // Multiple items: compact single-line with pipe delimiters to survive scanner stripping newlines
      // Format: HALTERUNG: <datum> || ART:<artnr>|BEZ:<bezeichnung>|M:<menge> || ART:<artnr>|BEZ:<bezeichnung>|M:<menge>
      const parts = items.map(i => `ART:${i.artikelnummer}|BEZ:${i.artikelbezeichnung.replace(/\|/g,'/')}-HALTERUNG|M:${i.menge}`);
      qrContent = `HALTERUNG: ${formattedDate} || ${parts.join(' || ')}`;
    }
    ...
    if (isSingleItem) {
      // Single item: Simple text layout (current format)
      zplParts.push(`^FO${X},${y}^AZN,${H6PT},${H6PT}^FDArtikelbezeichnung: ${items[0].artikelbezeichnung}^FS`); y += sp3;
      zplParts.push(`^FO${X},${y}^AZN,${H6PT},${H6PT}^FDArtikelnummer: ${items[0].artikelnummer}^FS`); y += sp3;
      zplParts.push(`^FO${X},${y}^AZN,${H6PT},${H6PT}^FDMenge: ${items[0].menge}^FS`); y += sp3;
      zplParts.push(`^FO${X},${y}^AZN,${H6PT},${H6PT}^FDVerpackungsdatum: ${formattedDate}^FS`);
    } else {
      // Multiple items: Table format like zubehoer
    ...
    console.error('Halterung API error:', err);
