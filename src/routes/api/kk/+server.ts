...
export async function PUT({ request }) {
    try {
		...
		// Build the exact QR payload you want for KK
        ...
        // --- layout helpers (same as C Pro) ---
        const mm = (n: number) => Math.round(n * 12);

        // inline GFA icons (same assets you used in C Pro)
        ...
        // Single-column detail rows (same order as C Pro style)
        `^AZN,${H6},${H6}`,
