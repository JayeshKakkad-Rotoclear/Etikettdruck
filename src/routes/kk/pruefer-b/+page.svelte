...
<form on:submit|preventDefault={loadSerialDataKK} class="form">
	...
	{#if serialErrorKK}<p class="error">{serialErrorKK}</p>{/if}
</form>  

{#if showFormKK}
    <form on:submit|preventDefault={submitFormKKB} class="form">
        ...
        <input id="qr_code_file" type="file" accept="image/svg+xml" on:change={handleFileUploadKK} />
